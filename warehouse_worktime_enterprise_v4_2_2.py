...
st.set_page_config(
    page_title="倉庫工時管理系統 V4.2.2 Enterprise",
    page_icon="🔐",
    layout="wide",
...
def normalize_role(value) -> str:
    """Normalize legacy/Chinese role values to the RBAC role keys."""
    if value is None:
        return "operator"
    cleaned = str(value).strip()
    if not cleaned:
        return "operator"
    return ROLE_ALIASES.get(cleaned, ROLE_ALIASES.get(cleaned.lower(), "operator"))
...
st.session_state["auth_user"] = user
allowed_pages = ROLE_PAGES.get(user["role"], ROLE_PAGES["operator"])

# Keep the active page valid after login or after a role change.
if st.session_state.get("current_page") not in allowed_pages:
    st.session_state["current_page"] = allowed_pages[0]

# Mobile navigation sets a pending page and reruns. Consume it before the
# sidebar radio is created so that the sidebar cannot overwrite the selection.
...
if pending_page in allowed_pages:
    ...
    st.session_state["fallback_page"] = pending_page
...
with st.sidebar:
    ...
    st.markdown(
        f'<span class="role-badge">{ROLE_LABELS.get(user["role"], "作業人員")}</span>',
        unsafe_allow_html=True,
    )
    ...
    # Desktop navigation remains authoritative when the user actively changes it.
    ...
    if st.button("🚪 登出", use_container_width=True):
        logout()
    st.caption("企業版 V4.2.2 Enterprise｜Supabase PostgreSQL｜Login & RBAC")

# Responsive fallback for phones and collapsed sidebars.
with st.expander("☰ 功能選單（手機／側邊欄收合時使用）", expanded=False):
    fallback_page = st.selectbox(
        "前往功能",
        allowed_pages,
        index=allowed_pages.index(st.session_state["current_page"]),
        key="fallback_page",
    ...
    if st.button(
        ...
        key="open_fallback_page",
    ):
        st.session_state["pending_page"] = fallback_page
        st.rerun()
...
st.title("倉庫工時管理系統 企業版 V4.2.2 Enterprise")
