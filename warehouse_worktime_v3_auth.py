...
st.set_page_config(
    page_title="倉庫工時管理系統 V3.0",
    page_icon="🔐",
    layout="wide",
    initial_sidebar_state="expanded",
)

DB_PATH = Path(os.getenv("WAREHOUSE_DB_PATH", "warehouse_v3.db"))
...
ROLE_PAGES = {
    "admin": ["首頁 Dashboard", "今日工時登錄", "工時明細查詢", "作業單據查詢", "工時日報表", "工時月報表", "圖表分析", "帳號與權限", "系統設定"],
    "manager": ["首頁 Dashboard", "今日工時登錄", "工時明細查詢", "作業單據查詢", "工時日報表", "工時月報表", "圖表分析"],
    "operator": ["首頁 Dashboard", "今日工時登錄", "我的工時明細", "作業單據查詢"],
    "viewer": ["首頁 Dashboard", "工時明細查詢", "作業單據查詢", "工時日報表", "工時月報表", "圖表分析"],
}
...
def fetch_logs() -> pd.DataFrame:
    with closing(conn()) as c:
        return pd.read_sql_query(
            "SELECT * FROM work_logs ORDER BY work_date DESC, start_time DESC", c
        )


def get_timer(username: str):
    with closing(conn()) as c:
        row = c.execute("SELECT * FROM active_timers WHERE username=?", (username,)).fetchone()
    return dict(row) if row else None


def start_timer(username: str, employee: str, work_type: str, document_no: str, note: str) -> None:
    with closing(conn()) as c:
        c.execute("DELETE FROM active_timers WHERE username=?", (username,))
        c.execute(
...
def pause_timer(username: str) -> None:
    with closing(conn()) as c:
        c.execute(
            "UPDATE active_timers SET pause_started_at=? WHERE username=? AND pause_started_at IS NULL",
    ...
    with closing(conn()) as c:
        c.execute(
            """UPDATE active_timers
               SET paused_seconds=paused_seconds+?, pause_started_at=NULL
               WHERE username=?""",
            (extra, username),
        )
        c.commit()
...
with st.sidebar:
    ...
    if st.button("🚪 登出", use_container_width=True):
        logout()
    st.caption("企業版 V3.0｜Login & RBAC")
...
if page == "首頁 Dashboard":
    ...
    with st.form("manual_entry"):
        ...
        c3, c4 = st.columns(2)
        start_time = c3.time_input("開始時間")
        end_time = c4.time_input("結束時間")
    ...
    with tab1:
        with closing(conn()) as c:
            users_df = pd.read_sql_query(
                "SELECT id,username,display_name,role,is_active,must_change_password,created_at,updated_at FROM users ORDER BY id", c
            )
        users_df["角色"] = users_df["role"].map(ROLE_LABELS)
        users_df["狀態"] = users_df["is_active"].map({1: "啟用", 0: "停用"})
        users_df["首次改密碼"] = users_df["must_change_password"].map({1: "是", 0: "否"})
        ...
        with st.form("edit_user"):
            ...
            role = st.selectbox("角色", list(ROLE_LABELS), index=list(ROLE_LABELS).index(selected["role"]), format_func=lambda x: ROLE_LABELS[x])
                    ...
                    if reset_password:
                        c.execute(
                            """UPDATE users SET display_name=?,role=?,is_active=?,password_hash=?,
                                                must_change_password=1,updated_at=? WHERE username=?""",
                            (display_name.strip(), role, int(active), hash_password(reset_password), now, selected_username),
                        )
                    else:
                        c.execute(
                            "UPDATE users SET display_name=?,role=?,is_active=?,updated_at=? WHERE username=?",
                            (display_name.strip(), role, int(active), now, selected_username),
                        )
                    c.commit()
    ...
    with tab3:
        with closing(conn()) as c:
            audit_df = pd.read_sql_query("SELECT * FROM audit_logs ORDER BY id DESC LIMIT 500", c)
    ...
    st.warning("Streamlit Community Cloud 的本機 SQLite 可能在重新部署或休眠後重建。公司正式使用請改接 PostgreSQL、Supabase 或 MySQL，並設定定期備份。")
    st.info("公開網址仍可被任何人開啟，但未通過帳號與密碼驗證者無法進入系統功能。")

st.divider()
st.caption("建議使用 Chrome 或 Microsoft Edge；請勿將初始密碼透過公開群組傳送。")
