...
/* Hide Streamlit owner/deployment controls from the app canvas.
   Streamlit may change these internal selectors in future releases. */
...
with st.sidebar:
    ...
    if st.button("🚪 登出", use_container_width=True):
        logout()
    st.caption("企業版 V4.1｜Supabase PostgreSQL｜Login & RBAC")
...
if page == "首頁 Dashboard":
    ...
    with st.form("manual_entry"):
        ...
        input_mode = st.radio(
            ...
            ["手動 Key in", "下拉選擇（時／分）"],
            horizontal=True,
            help="可隨時切換輸入方式。手動輸入請使用 HH:MM 格式。",
        )
        ...
        if input_mode == "手動 Key in":
            c3, c4 = st.columns(2)
            start_text = c3.text_input("開始時間（HH:MM）", value=default_start.strftime("%H:%M"), placeholder="08:30")
            end_text = c4.text_input("結束時間（HH:MM）", value=default_end.strftime("%H:%M"), placeholder="17:30")
            try:
            ...
            with c3:
                st.markdown("**開始時間**")
                start_time = dropdown_time("manual_start", default_start)
            with c4:
                st.markdown("**結束時間**")
                end_time = dropdown_time("manual_end", default_end)
