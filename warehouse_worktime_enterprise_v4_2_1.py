...
st.set_page_config(
    page_title="倉庫工時管理系統 V4.2.1 Enterprise",
    page_icon="🔐",
    layout="wide",
...
with st.sidebar:
    ...
    sidebar_page = st.radio(
        "功能",
        allowed_pages,
        index=allowed_pages.index(st.session_state.get("current_page", allowed_pages[0]))
        if st.session_state.get("current_page") in allowed_pages else 0,
    ...
    if st.button("🚪 登出", use_container_width=True):
        logout()
    st.caption("企業版 V4.2.1 Enterprise｜Supabase PostgreSQL｜Login & RBAC")

# Responsive fallback: on phones or when the sidebar is collapsed, users can still navigate.
with st.expander("☰ 功能選單（手機／側邊欄收合時使用）", expanded=False):
    ...
    if st.button("開啟功能", type="primary", use_container_width=True, key="open_fallback_page"):
        st.session_state["current_page"] = fallback_page
        st.rerun()

page = st.session_state.get("current_page", allowed_pages[0])
...
st.title("倉庫工時管理系統 企業版 V4.2.1 Enterprise")
